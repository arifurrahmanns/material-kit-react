...
import { UserListHead, UserListToolbar, UserMoreMenu } from '../../sections/@dashboard/vendor';
...
const TABLE_HEAD = [
    ...
    { id: 'company', label: 'Owner', alignRight: false },
...
export default function Vendors() {
    ...
    useEffect(() => {
        axios.get("/admin/vendors/get?with_owner=true&page=" + (page + 1)).then((response) => {
    ...
    const handleValueChange = () => {
        ...
        axios.get("/admin/vendors/get?with-owner=true&search=" + filterName).then((response) => {
            if (response.data.success) {
                setUSERLIST([...response.data.success.data])
                setTotalPage(response.data.success.total)
                setRowsPerPage(response.data.success.per_page)
                                ...
                                <Table stickyHeader >
                                    ...
                                    <TableBody>
                                        {filteredUsers
                                            ...
                                            .map((row) => {
                                                const { id, vendorName, avatar, vendorEmail, vendorPhone, status, user } = row;
                                                    ...
                                                    >
                                                        ...
                                                        <TableCell component="th" scope="row" padding="none">
                                                            <Stack direction="row" alignItems="center" spacing={2}>
                                                                <Avatar alt={vendorName} src={getFilename(avatar, 50)} />
                                                                <Typography variant="subtitle2" noWrap>
                                                                    {vendorName}
                                                                </Typography>
                                                            </Stack>
                                                        </TableCell>
                                                        <TableCell align="left">{user !== null ? user.firstName + " " + user.lastName :
                                                            <Label
                                                                variant="ghost"
                                                                color={'error'}
                                                            >
                                                        ...
                                                        <TableCell align="left">{vendorEmail}</TableCell>
                                                        <TableCell align="left">{vendorPhone}</TableCell>
                                                        ...
                                                        <TableCell align="right">
                                                            <IconButton component={RouterLink} to={"/dashboard/vendor/" + id}>
